...
# proc. data -> year
info$year <- as.numeric(sapply(info$NUCCORE_CreateDate, function(x){ unlist(strsplit(x, '-'))[1] }))
...
for(r in ranks){
    num_taxa <- rbind(num_taxa, data.frame(
        rank=r,
        count=length(unique(na.omit(info[,sprintf('TAXONOMY_%s_id', r)])))
    ))
}
...
print(summary(info$NUCCORE_Length))
print('GC summary')
print(summary(info$NUCCORE_GC))

# How many have (mapped) location information
print(sprintf('With location information: %d', sum(!is.na(info$loc_lat))))

# How many have ( mapped)host information
print(sprintf('With host information: %d', sum(!grepl("unknown", info$BIOSAMPLE_Host_label))))
...
# plot:
plot_length_gc <-   ggplot(data=info, aes(x=NUCCORE_Length, y=NUCCORE_GC)) +
