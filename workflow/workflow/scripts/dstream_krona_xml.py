    ...
    for taxon, taxon_df in df.groupby(by='new_%s' % rank):
        ...
        for db in labels:
            if db == 'All':
                ...
                ET.SubElement(taxon_count, 'val').text = str(taxon_df.loc[taxon_df['NUCCORE_Source'] == db,:].shape[0])
...
if __name__ == "__main__":
    ...
    # Logger setup
    logger = My_logger(log_filename = args.log, logger_name = "dstream_krona_xml")
    ...
    # labels
    labels = ['All'] + sorted(set(tab['NUCCORE_Source']))

    # create new taxon names: "taxon <name> (<taxon ID>)"
    for rank in ranks:
        tab['new_%s' % rank] = tab[['TAXONOMY_%s' % rank, 'TAXONOMY_%s_id' % rank]].apply(new_taxon_name, axis=1)
        my_logger.info('Processed {} taxa'.format(rank))

    ##############################
    # XML START
    my_logger.info('Create XML tree structure...')
    ...
    for db in labels:
        if db == 'All':
            ...
            ET.SubElement(root_count, 'val').text = str(tab.loc[tab['NUCCORE_Source'] == db,:].shape[0])
    # nodes for ranks
