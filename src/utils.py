...
def preproc_loc_coords(loc_str):
    ...
    if re.fullmatch(r'[0-9]+(\.[0-9]+)?\s?[N|S][\s,]*[0-9]+(\.[0-9]+)?\s?[W|E]', loc_str):
        matches = [prepend_dotzero(re.sub(' ','',m.group())) for m in re.finditer(r'[0-9]+(\.[0-9]+)?\s?[N|S|W|E]', loc_str)]
...
def parse_location(loc_name, loc_coords, api_key):
    ...
    loc = None
    # exceptions
    if loc_name in location_exceptions:
        loc_name = location_exceptions[loc_name]
    if loc_coords in location_exceptions:
        loc_coords = location_exceptions[loc_coords]
    # try name
