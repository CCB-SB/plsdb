...
def get_arg_parser():
    ...
    parser.add_argument('--icol', '-i', help="Column name containing sequnece IDs (same for all files)", required=True)
...
##################################################
ARGS = None
CMD  = "{epath}/epost -db nuccore -id \"{ids}\" | {epath}/efetch -format fasta > {ofile}"
...
if __name__ == "__main__":
    ...
    tmp = pool.starmap(
        download_sequences,
        enumerate([chunk for chunk in chunk_list(ids, num=None, length=ARGS.size)])
    )
    pool.close()
    pool.join()
